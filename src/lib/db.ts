...
export function getDb() {
  // Use DATABASE_URL first (the Neon pooler URL with all our data)
  const url = process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.POSTGRES_PRISMA_URL || process.env.POSTGRES_URL_NON_POOLING || process.env.STORAGE_URL;
...
0% refundable if canceled after.'`;
  ...
  `;
  await sql`ALTER TABLE email_templates ADD COLUMN IF NOT EXISTS property_slug TEXT NOT NULL DEFAULT 'both'`;
  const tplNow = new Date().toISOString();
  const bookingConfirmedBody = 'Hi {{guest_name}},\n\nYour booking is confirmed! Here are your details:\n\nProperty: {{property_name}}\nCheck-in: {{check_in}} after 4:00 PM\nCheck-out: {{check_out}} before 10:00 AM\nNights: {{nights}}\nTotal paid: ${{total}}\nBooking ID: {{booking_id}}\n\nYou will receive check-in instructions including door code, parking, and WiFi details 3 days before your arrival.\n\nBefore your stay, please review your rental agreement: {{rental_agreement_url}}\n\nWe look forward to welcoming you to Lake Tahoe!\n\nGrace & Andrew\nGDP Tahoe\ngdpgroup20@gmail.com | 603-359-9227\nstaygdptahoe.com';
  const ownerNotificationBody = 'New direct booking received!\n\nGuest: {{guest_name}}\nProperty: {{property_name}}\nCheck-in: {{check_in}}\nCheck-out: {{check_out}}\nNights: {{nights}}\nTotal: ${{total}}\nBooking ID: {{booking_id}}\n\nView in portal: https://www.staygdptahoe.com/admin';
  const preCheckinBodyElevation = 'Hi {{guest_name}},\n\nYour stay at {{property_name}} is just 3 days away — we\'re so excited to host you!\n\nCHECK-IN INFORMATION\nAddress: 6229 N Lake Blvd, Tahoe Vista, CA 96148\nCheck-in: After 4:00 PM on {{check_in}}\nDoor code: [Add door code here]\nParking: [Add parking instructions here]\nWiFi: [Add WiFi name and password here]\n\nDURING YOUR STAY\nIf you need anything, reach us at 603-359-9227 (call or text) or reply to this email.\n\nRECOMMENDATIONS\nWe\'ve put together our favorite local restaurants, activities, and services at:\n{{recommendations_url}}\n\nPlease remember:\n• Quiet hours are 10 PM – 7 AM\n• No parties or large gatherings\n• Lock all doors when leaving\n\nWe hope you have an incredible time at Lake Tahoe!\n\nGrace & Andrew\nGDP Tahoe';
  const preCheckinBodyTurquoise = 'Hi {{guest_name}},\n\nYour stay at {{property_name}} is just 3 days away — we\'re so excited to host you!\n\nCHECK-IN INFORMATION\nAddress: 5233 Turquoise Ave, Carnelian Bay, CA 96140\nCheck-in: After 4:00 PM on {{check_in}}\nDoor code: [Add door code here]\nParking: [Add parking instructions here]\nWiFi: [Add WiFi name and password here]\n\nDURING YOUR STAY\nIf you need anything, reach us at 603-359-9227 (call or text) or reply to this email.\n\nRECOMMENDATIONS\nWe\'ve put together our favorite local restaurants, activities, and services at:\n{{recommendations_url}}\n\nPlease remember:\n• Quiet hours are 10 PM – 7 AM\n• No parties or large gatherings\n• Lock all doors when leaving\n\nWe hope you have an incredible time at Lake Tahoe!\n\nGrace & Andrew\nGDP Tahoe';
  const postCheckoutBody = 'Hi {{guest_name}},\n\nThank you for staying at {{property_name}}! We hope you had a wonderful time in Lake Tahoe.\n\nIf you enjoyed your stay, we\'d be so grateful if you could leave us a review — it makes a huge difference for our small family business.\n\nLeave a review on Airbnb: [Add your Airbnb review link]\nLeave a review on VRBO: [Add your VRBO review link]\n\nWe hope to welcome you back soon. If you\'d like to book your next stay directly and save on service fees, visit us at:\nhttps://www.staygdptahoe.com\n\nWith gratitude,\nGrace & Andrew\nGDP Tahoe';
  // Delete old non-property-specific templates
  await sql`DELETE FROM email_templates WHERE id IN ('tpl-booking-confirmed', 'tpl-owner-notification', 'tpl-pre-checkin', 'tpl-post-checkout')`;
  // Seed 8 per-property templates (4 types × 2 properties)
  const tplSeeds = [
    { id: 'tpl-booking-confirmed-elevation', name: 'Guest Booking Confirmation — Elevation', trigger: 'booking_confirmed', subject: 'Your stay at {{property_name}} is confirmed!', body: bookingConfirmedBody, daysOffset: 0, slug: 'elevation-estate' },
    { id: 'tpl-booking-confirmed-turquoise', name: 'Guest Booking Confirmation — Turquoise', trigger: 'booking_confirmed', subject: 'Your stay at {{property_name}} is confirmed!', body: bookingConfirmedBody, daysOffset: 0, slug: 'turquoise' },
    { id: 'tpl-owner-notification-elevation', name: 'Owner Notification — Elevation', trigger: 'owner_notification', subject: 'New booking: {{guest_name}} — {{property_name}} {{check_in}} to {{check_out}}', body: ownerNotificationBody, daysOffset: 0, slug: 'elevation-estate' },
    { id: 'tpl-owner-notification-turquoise', name: 'Owner Notification — Turquoise', trigger: 'owner_notification', subject: 'New booking: {{guest_name}} — {{property_name}} {{check_in}} to {{check_out}}', body: ownerNotificationBody, daysOffset: 0, slug: 'turquoise' },
    { id: 'tpl-pre-checkin-elevation', name: 'Pre-Arrival Instructions — Elevation', trigger: 'pre_checkin', subject: 'Your stay at {{property_name}} is coming up — everything you need to know', body: preCheckinBodyElevation, daysOffset: -3, slug: 'elevation-estate' },
    { id: 'tpl-pre-checkin-turquoise', name: 'Pre-Arrival Instructions — Turquoise', trigger: 'pre_checkin', subject: 'Your stay at {{property_name}} is coming up — everything you need to know', body: preCheckinBodyTurquoise, daysOffset: -3, slug: 'turquoise' },
    { id: 'tpl-post-checkout-elevation', name: 'Post-Stay Review Request — Elevation', trigger: 'post_checkout', subject: 'Thank you for staying with GDP Tahoe — we\'d love your feedback', body: postCheckoutBody, daysOffset: 1, slug: 'elevation-estate' },
    { id: 'tpl-post-checkout-turquoise', name: 'Post-Stay Review Request — Turquoise', trigger: 'post_checkout', subject: 'Thank you for staying with GDP Tahoe — we\'d love your feedback', body: postCheckoutBody, daysOffset: 1, slug: 'turquoise' },
  ];
  for (const t of tplSeeds) {
    await sql`
      INSERT INTO email_templates (id, name, trigger, subject, body, days_offset, enabled, updated_at, property_slug)
      VALUES (${t.id}, ${t.name}, ${t.trigger}, ${t.subject}, ${t.body}, ${t.daysOffset}, 1, ${tplNow}, ${t.slug})
      ON CONFLICT (id) DO NOTHING
    `;
  }
